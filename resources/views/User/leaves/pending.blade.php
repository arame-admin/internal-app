    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex gap-1 mb-6 border-b border-gray-200">
            <a href="{{ route('manager.leaves.approve') }}" 
                ...
                {{ request()->is('*leaves/approve*') || request()->is('*team-history*') ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                History
            ...
            <a href="{{ route('manager.leaves.pending') }}" 
                ...
                Pending
                <span class="ml-2 px-2 py-0.5 bg-yellow-100 text-yellow-800 text-xs rounded-full">{{ $pendingLeaves->count() }}</span>
            </a>
        </div>
...
<script>
    function showRejectModal(leaveId) {
        ...
        form.action = '{{ route("manager.leaves.approve.update", ":id") }}'.replace(':id', leaveId);
