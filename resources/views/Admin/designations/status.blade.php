...
@section('title', 'Change Designation Status')

@section('content')
<!-- Status Toggle Card -->
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <!-- Designation Info Header -->
        <div class="bg-gradient-to-r from-blue-500 to-indigo-600 p-6">
            <div class="flex items-center space-x-4">
                ...
                <div class="text-white">
                    <h2 class="text-xl font-bold">{{ $designation->name }}</h2>
                    <p class="text-blue-100 text-sm">{{ $designation->code }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <div class="flex items-center space-x-2">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $designation->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            <span class="w-2 h-2 {{ $designation->status === 'active' ? 'bg-green-500' : 'bg-red-500' }} rounded-full mr-2"></span>
                ...
                <div class="text-right">
                    ...
                    <p class="text-lg font-semibold text-gray-800">{{ $designation->created_at->format('M d, Y') }}</p>
                </div>
            </div>
        </div>

        <!-- Toggle Form -->
        <form action="{{ route('admin.designations.status.update', Crypt::encrypt($designation->id)) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <label class="relative cursor-pointer group">
                        <input type="radio" name="status" value="active" class="peer sr-only" {{ $designation->status === 'active' ? 'checked' : '' }}>
                        <div class="p-4 rounded-xl border-2 border-gray-200 peer-checked:border-green-500 peer-checked:bg-green-50 transition-all group-hover:border-green-300">
                            ...
                            <p class="text-sm text-gray-500 mt-1">Designation will be fully functional and assignable to users</p>
                        </div>
                    </label>

                    <!-- Inactive Option -->
                    <label class="relative cursor-pointer group">
                        <input type="radio" name="status" value="inactive" class="peer sr-only" {{ $designation->status === 'inactive' ? 'checked' : '' }}>
                        <div class="p-4 rounded-xl border-2 border-gray-200 peer-checked:border-red-500 peer-checked:bg-red-50 transition-all group-hover:border-red-300">
                            ...
                            <p class="text-sm text-gray-500 mt-1">Designation will be disabled and cannot be assigned</p>
                        </div>
                    </label>
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea id="reason" name="reason" rows="2" placeholder="Enter reason for status change..." class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all resize-none">{{ old('reason') }}</textarea>
            </div>
            ...
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                <div class="flex items-start space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-yellow-700 mt-1">If you set this designation to inactive, it will not be available for new assignments.</p>
                    </div>
                </div>
            </div>
