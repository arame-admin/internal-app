...
{{-- This page displays a list of locations with search, filter, sort, and pagination functionality. --}}
...
<div class="bg-gradient-to-br from-blue-50 via-white to-indigo-50 min-h-screen py-8">
    <div class="max-w-7xl mx-auto px-4">
        ...
        <!-- Floating Add Button -->
        <a href="{{ route('admin.locations.create') }}" class="fixed bottom-6 right-6 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-full p-4 shadow-lg hover:from-blue-600 hover:to-indigo-700 transition-colors z-50" title="Add New Location">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
            <div class="p-4 border-b border-gray-100">
                <form action="{{ route('admin.locations.index') }}" method="GET" id="search-form" class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <!-- Search -->
                    <div class="relative flex-1 max-w-md">
                        <svg class="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                        </svg>
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search locations..." class="w-full pl-10 pr-12 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" id="search-input">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left">
                                ...
                                @endphp
                                <a href="{{ route('admin.locations.index', ['sort' => $column, 'direction' => $newDirection] + request()->except(['sort', 'direction'])) }}" class="text-xs font-semibold text-gray-500 uppercase tracking-wider hover:text-gray-700 flex items-center space-x-1">
                                    <span>Name</span>
                            ...
                            <th class="px-6 py-4 text-left">
                                <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider">City</span>
                            </th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <p class="text-sm text-gray-600 max-w-xs truncate">{{ $location->address }}</p>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end space-x-2">
                                    <button onclick="toggleStatus('{{ Crypt::encrypt($location->id) }}', '{{ $location->status }}', this)" class="p-2 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="Change Status">
                                        @if($location->status == 'active')
                                    ...
                                    <a href="{{ route('admin.locations.edit', Crypt::encrypt($location->id)) }}" class="p-2 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if($locations->hasPages())
                <div class="px-6 py-4 border-t border-gray-100 pagination-container">
                    <p class="pagination-info">
                        Showing {{ ($locations->currentPage() - 1) * $locations->perPage() + 1 }} to
                        {{ min($locations->currentPage() * $locations->perPage(), $locations->total()) }}
                        of {{ $locations->total() }} locations
                    </p>
                    ...
                    <div class="pagination">
                        ...
                        @else
                            <a href="{{ $locations->previousPageUrl() }}" class="page-item">
                                <span class="page-link page-btn">&lsaquo;</span>
                            </a>
                        @endif
                        ...
                        @for ($i = 1; $i <= $locations->lastPage(); $i++)
                            ...
                            @else
                                <a href="{{ $locations->url($i) }}" class="page-item">
                                    <span class="page-link">{{ $i }}</span>
                                </a>
                            @endif
                        @endfor
                        ...
                        @if ($locations->hasMorePages())
                            <a href="{{ $locations->nextPageUrl() }}" class="page-item">
            ...
            @else
                <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
                    <p class="text-sm text-gray-500">Showing {{ $locations->count() }} of {{ $locations->total() }} locations</p>
                </div>
            @endif
        </div>
    </div>
...
$(document).ready(function() {
    ...
    window.toggleStatus = function(encryptedId, currentStatus, button) {
        ...
        $.ajax({
            url: '/admin/locations/' + encryptedId + '/status',
            type: 'PUT',
            data: {
                status: newStatus,
                _token: '{{ csrf_token() }}'
